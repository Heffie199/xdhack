...
class UsernameLogin:
    def __init__(self,username,ua,TPL_password2):
        ...
        '''
        #检查是否需要验证码的URL
        self.nick_check_url = 'https://login.jd.com/member/request_nick_check.do?_input_charset=utf-8'
        #验证京东用户名密码的URL
        self.login_url = 'https://login.jd.com/member/login.jhtml'
        #访问st码URL
        self.vst_url = 'https://login.jd.com/member/vst.htm?st={st}'

        #京东用户名
        self.username = username
        #京东关键参数,包含用户浏览器等一些信息,很多地方会使用,从浏览器或抓取工具包中复制,可重复使用
        self.ua = ua
...
def login(self):
    ...
    if my_taobao_match:
        print('登录京东成功，跳转链接：{}'.format(my_taobao_match.group(1)))
...
def get_taobao_nick_name(self):
    ...
    if nick_name_match:
        print('登录京东成功，你的用户名是：{}'.format(nick_name_match.group(1)))
        return nick_name_match.group(1)
    else:
        raise RuntimeError('获取京东昵称失败！response：{}'.format(response.text))


if __name__ == '__main__':
    # 京东用户名
    username = '你的用户名'
    # 京东重要参数，从浏览器或抓包工具中复制，可重复使用
